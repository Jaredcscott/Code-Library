...
public class Puzzle {
    ...
    public void solve(boolean doPrint) {
        ...
        LinkedList.insert(allNodes, this.initNode);
        LinkedList.ListNode curNode = allNodes.head;
        Hashtable<Integer, Integer> foundNodes = new Hashtable<>();
        int count = 0; 
        Node[] newNodes;
        while (!(curNode.data.isGoal())) {
            newNodes = curNode.data.expand();
            for (Node newNode : newNodes) {
                if (!(foundNodes.contains(newNode.hashCode()))) {
                    LinkedList.insert(allNodes, newNode);
                    foundNodes.put(count, newNode.hashCode());
                    count++;
                }
            }
            curNode = curNode.next;
            LinkedList.removeHead(allNodes);
        }
        Node solution= curNode.data;
        System.out.print("\n\nSOLUTION  of Depth " + solution.getDepth());
        System.out.println(" Total Nodes Expanded " + allNodes.ListLength + "\n");
